import { Alert, Button, Card, Col, Form, Input, InputNumber, Row, Select, Space, Tag, Tree, Typography, message } from 'antd';
import type { DataNode } from 'antd/es/tree';
import { PlusOutlined, ReloadOutlined, SaveOutlined } from '@ant-design/icons';
import { useEffect, useMemo, useState } from 'react';
import type { CreateOrgRequest, DataScope, OrgNode, TenantRole } from '@xiaoa/share/types';
...
function treeData(nodes: OrgNode[]): DataNode[] {
  return nodes.map((node) => ({ key: String(node.id), title: <Space size={8}><Tag>{orgLabels[node.type]}</Tag><span>{node.name}</span><Typography.Text type="secondary">#{node.id}</Typography.Text></Space>, children: node.children?.length ? treeData(node.children) : undefined }));
}
